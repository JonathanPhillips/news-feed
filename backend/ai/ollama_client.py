...
class LMStudioClient:
    ...
    def categorize_article(self, title: str, content: str, model: str = None) -> Dict[str, Any]:
        ...
        if model is None:
            try:
                ...
                model = "mistralai/mistral-7b-instruct-v0.3"  # fallback to known model
        
        prompt = f"""Analyze this news article and provide a JSON response with the following structure:
{{
    "category": "one of: technology, politics, business, science, health, sports, entertainment, world",
    "sentiment": "positive, negative, or neutral",
    "importance": "high, medium, or low",
    "topics": ["list", "of", "key", "topics"],
    "summary": "brief one-sentence summary",
    "political_bias": "number from -1.0 (strong left bias) to 1.0 (strong right bias), 0 = neutral/non-political",
    "bias_confidence": "number from 0.0 to 1.0 indicating confidence in bias assessment"
}}
...
Consider language choice, framing, source selection, and perspective when assessing bias.
...
Respond only with valid JSON, no other text."""
        ...
        try:
            response = openai.ChatCompletion.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a news article analyzer. Respond only with valid JSON."},
    ...
    def summarize_article(self, content: str, model: str = None) -> str:
        ...
        try:
            response = openai.ChatCompletion.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a news summarizer. Provide concise, factual summaries."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=100,
                temperature=0.1
